...
function List_Order() {
    ...
    useEffect(()=>{
        rq_customer.get('/order/list_order')
        .then(
            res=>{
              if(res.data.denied)
              navigate('/denied')
  ...
  <div className={cx('wrapper')}>
    ...
    <div className={cx('menu')}>
      {status.map((item)=>{
        return(
        <button key={item.id} onClick={()=>{setActive(item.id)}} className={cx('btn-menu-item',`${activeStatus === item.id && 'active'}`)}>{item.message} <FontAwesomeIcon icon={item.icon} ></FontAwesomeIcon></button>)
      })}
    </div>
              ...
              return (<tr key={index}>
                ...
                <td className={cx('td','mark')}>
                    ...
                    <Link className={cx('btn-action-red')}>
                      <button onClick={()=>{
                        rq_customer.get(`/order/cancel/${order._id}`)
                        .then(res=>{
                          if(res.data.updated)
                          navigate(0)
                        })
                      }} className={cx('btn-delete')}>Hủy đơn</button>
                    </Link>
                </td>
              </tr>)
                })}    
